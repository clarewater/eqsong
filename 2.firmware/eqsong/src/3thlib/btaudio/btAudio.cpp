...
arm_biquad_casd_df1_inst_f32 s_left;
arm_biquad_casd_df1_inst_f32 s_right;

// 然后初始化静态成员指针
arm_biquad_casd_df1_inst_f32 btAudio::s_left = s_left;
arm_biquad_casd_df1_inst_f32 btAudio::s_right = s_right;
...
////////////////////////////////////////////////////////////////////
void btAudio::I2S(int mck, int bck, int dout, int ws)
{
  // i2s configuration
...
void btAudio::i2sCallback(const uint8_t *data, uint32_t len)
{
  ...
  case EQ_FILTER:
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      // 对左声道和右声道进行处理
      arm_biquad_cascade_df1_int16(&btAudio::s_left, &fy[0], temp, 1);
      arm_biquad_cascade_df1_int16(&btAudio::s_right, &fy[1], temp + 1, 1);
      i2s_write(I2S_NUM_0, temp, jump, &i2s_bytes_write, 100);
    }
    break;
  }
}

void btAudio::setPostProcess(int postprocess)
{
...
void btAudio::setDSP_iir_left(
    uint8_t numStages,
    float32_t *pCoeffs,
    float32_t *pState)
{
  arm_biquad_cascade_df1_init_int16(
      &btAudio::s_left,
      numStages,
      pCoeffs,
      pState);
